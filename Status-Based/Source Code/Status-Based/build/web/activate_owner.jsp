...
<html>
    ...
    session.setAttribute("semail", mail);
     try {
        ...
        int i = st.executeUpdate("update ownerreg set status = '" + "Activated" + "' where email='" + mail + "'");
        if (i != 0) {
          out.println("<script>alert('Activated Successfully')</script>");
           
            response.sendRedirect("active_owner.jsp?msg=Activated");
        } else {
            response.sendRedirect("active_owner.jsp?msg=Failed");
        }
