...
throws IOException{ 
    response.setContentType("text/html;charset=UTF-8");
        PrintWriter out = response.getWriter();
            ...
            pstmt = con.prepareStatement("insert into pro (filename, size, file, data) values(?,?,?,?)");
            ...
            if(i!=0){
              // response.sendRedirect("f_upload.jsp?fileuplodedsusscessfully...");
                out.println("<script>alert('File Upload to cloud')</script>");
            }
            else{
                response.sendRedirect("fileupload1.jsp?Uploadingfailed...");
            }
        } catch(Exception ex){
             response.sendRedirect("fileupload1.jsp?File_name_already_exit");
            ex.printStackTrace();
        } 
                finally{
                    out.close();
